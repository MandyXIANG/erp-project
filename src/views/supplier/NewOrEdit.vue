  ...
  <div class="new-customer-container">
    ...
    <el-collapse v-model="activeName">
      ...
      <el-collapse-item name="5">
        ...
        <PayMethod ref="child" :labelWidth="120" :payMethod="payMethod" />
      </el-collapse-item>
    </el-collapse>
    <div class="btm-area">
...
import Basic from '../customer/components/Basic'

import RelationInfo from '../customer/components/RelationInfo.vue'
import FinancialInfo from '../customer/components/FinancialInfo'
...
export default {
  ...
  data () {
    return {
      ...
      personTableCol,
      relationForm,
      payMethod: _.cloneDeep(payMethods), // 结算方式
      rules: {},
      codes: [
  ...
  methods: {
    initForm () {
      ...
      if (this.selectId) {
        ...
        this.pageTitle = '新增供应商'
        this.form = {}
        this.financialForm = {}
        this.dynamicForm = _.cloneDeep(DynamicForm)
        this.payMethod = _.cloneDeep(payMethods)
        this.FdynamicForm = _.cloneDeep(FdynamicForm)
      }
      this.form = { ...this.form, ...this.dynamicForm }
      this.financialForm = Object.assign(this.financialForm, this.FdynamicForm)
    },
    getItemInfo () {
    ...
    handleFormData () {
      ...
      this.payMethod = cpyForm.settles

      const stage = cpyForm.settles.stage || {}
      const cycle = cpyForm.settles.cycle || {}
      if (!stage.stage_conf || !stage.stage_conf.length) {
        this.payMethod.stage.stage_conf = [{ ratio: null, time: '预付' }]
      }
      if (!cycle.cycle_conf || !cycle.cycle_conf.length) {
        this.payMethod.cycle.cycle_conf = [{ unit: 'week', value: '' }]
      }
      this.$refs.child.paymentChange(null, this.payMethod)
    },
    goBack () {
      this.$router.go(-1)
    },
    onSubmit () {
      ...
      if (!this.selectId) {
        this.$http
          .post(API_MAP.BASE_SUPPLIER, { data: params, error_return: true })
          .then(res => {
            if (res.code === 0) {
          ...
          .catch(err => {
            this.btnLoading = false
            if (err.code === 15) {
              this.$message.error(`供应商代码${this.form.code}已存在`)
            } else if (err.code === 16) {
              this.$message.error(`供应商代码${this.form.code}重复使用`)
            } else {
        ...
        this.$http
          .put(API_MAP.BASE_SUPPLIER, { data: params, error_return: true })
          .then(res => {
            if (res.code === 0) {
              this.$message.success('操作成功')
              this.btnLoading = false
